...
class cls_raintpl{
	...
	//check cache id if that was null we set that to localize language
	if($cache_id == null){
		$this->cache_id = $this->localize->get_language();
	}
	else{
		// set the cache_id
		...
		if( !$this->check_template( $tpl_name ) && file_exists( $this->tpl['cache_filename'] ) && ( time() - filemtime( $this->tpl['cache_filename'] ) < $expire_time ) ){
			return file_get_contents( $this->tpl['cache_filename'] );
		}
		else{
