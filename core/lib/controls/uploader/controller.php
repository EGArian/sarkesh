		...
		elseif ($service == 'upload') {
			if(isset($_REQUEST['id'])){
				...
				if(isset($_FILES[$_REQUEST['id'] . '_uploader'])){
					...
					if(1==1){
						//all is fine going to move uploaded file
						$address = AppPath . 'upload/files/' . cls_general::random_string(10,'NC') . '.' . $_REQUEST['type'];
						move_uploaded_file($_FILES[$_REQUEST['id'] . '_uploader']['tmp_name'], $address);
						echo $address;
					}
				}
			}				
