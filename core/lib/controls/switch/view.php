...
class ctr_uploader_view{
	...
	public function view_draw($uploader){
		...
		$this->raintpl->assign( "php_progressbar_request_name", ini_get("session.upload_progress.name"));
		$this->raintpl->assign( "php_progressbar_request_value", $uploader['NAME']);
		$this->raintpl->assign( "label_select_file", _('Select File'));
		$this->raintpl->assign( "form_name", $uploader['FORM']);
		//checking unit
