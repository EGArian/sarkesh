...
class cls_mail {
	...
	public function simple_send($to_name, $to_email, $subject, $body){
		...
		try {
			$this->setFrom($localize_data['name'], $localize_data['email']);
			$this->addRecipient($to_name, $to_email);
			$this->fillSubject($subject);
			$this->fillMessage($body);
			// now we send it!
			$this->send();
			return true;
		 }
...
try {
	...
	$dummy->addRecipient("alizadeh.babak","alizadeh.babak@gmail.com");
