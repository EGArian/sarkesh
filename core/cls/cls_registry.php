<?php
	...
	class cls_registry{
		...
		public function get_plugin($plugin){
			 $this->db->do_query('SELECT r.a_key, r.value, p.name FROM ' . TablePrefix . 'registry r INNER JOIN plugins p ON p.id = r.plugin  WHERE p.name = ?;', array($plugin));
		...
		public function set($plugin, $key, $value){
			$this->db->do_query('UPDATE ' . TablePrefix . 'registry SET value=? WHERE plugin=? and a_key=?;', array($value, $plugin, $key)); 
		}
		
		public function delete_plugin($plugin){
			//this function delete all plugin keys
			$this->db->do_query('DELETE FROM ' . TablePrefix . 'registry WHERE plugin=?;', array($plugin)); 
		...
		public function add_key($plugin, $key){
			 ...
			 if( gettype($result) == 'boolean' ){
				...
				$this->db->do_query('INSERT INTO ' . TablePrefix . 'registry (plugin,a_key) VALUES (?,?);', array($plugin, $key)); 
