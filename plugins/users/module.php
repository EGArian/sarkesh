...
class users_module{
	...
	public function check_email($email = null){
		...
		//going to check
		$this->db->do_query('SELECT email FROM users WHERE email=?;', array($email));
	...
	public function get_user_info($value, $parameter = 'email'){
		$this->db->do_query('SELECT * FROM users WHERE ' . $parameter . '=?;', array($value));
	...
	public function get_current_user_info($username = ''){
		if($username == ''){
			...
			if($this->validator->is_set('USERS_LOGIN')){
				//user is loged in before now we want to find his/her permission id
				$id = $this->validator->get_id('USERS_LOGIN','id');
				$this->db->do_query('SELECT * FROM users WHERE validator=?;', array($id));
				return $this->db->get_first_row_array();
			...
			$this->db->do_query('SELECT * FROM users WHERE username=?;', array($username));
			return  $this->db->get_first_row_array();		
		}
	...
	public function reset_password($id){
		...
		if($this->db->do_query('UPDATE users SET password=?, forget=0 WHERE Forget=?;', array(md5($password), $id))){
	...
	public function is_registered_email($email){
		$this->db->do_query('SELECT * FROM users WHERE email =?;', array($email));
		if($this->db->rows_count() != 0){
			return true;
		}
		else{
	...
	public function register($username, $password, $email){
		...
		if($this->is_registered_email($email) == false && $this->is_registered_username($username) == false){
			...
			if($registry->get('users', 'active_from_email') != '1'){
				$permission_id = $registry->get('users', 'default_permission');
			}
			//save user
			$this->db->do_query('INSERT INTO users (username,password,email,permission) VALUES (?,?,?,?);', array($username, md5($password), $email,$permission_id));
			...
			if($permission_id == '3'){
			      ...
			      $this->db->do_query('UPDATE users SET validator=? WHERE username=?;', array($validator['id'], $username));
			      //going to send email
	...
	//this function check permission in permission table
	//$permission is column name on permission table
	public function has_permission($permission = 'enable', $username = ''){
		//first get user id
		if($username == ''){
			...
			if($user_info == '-1'){
				//user is guest
				$this->db->do_query('SELECT * FROM permissions where id=4;');
			}
			else{
				$this->db->do_query('SELECT * FROM permissions where id=?;', array($user_info['permission']));
			}
			...
			//user not enabled so no permission to access to system
			return false;
		}
