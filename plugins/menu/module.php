...
class menu_module{
	...
	public function get_menus($position){
		...
		//first get menus
		$query = 'SELECT * FROM ' . TablePrefix . 'menus where position=? and localize=?;';
		...
		foreach($result as $menu){
			 ...
			 $query .= " FROM " . TablePrefix . "menus m INNER JOIN links l ON l.ref_id = m.id WHERE m.position=? and m.localize=? and l.ref_id=?;"; 
